...
namespace SuperPutty.Data.Sources
{
    ...
    public sealed class NetworkBrowser
    {
        ...
        public List<_SERVER_INFO_100> getNetworkComputers()
        {
            //local fields
            List<_SERVER_INFO_100> networkComputers = new List<_SERVER_INFO_100>();
    ...
    [XmlType("SessionWindowsNetworkSource")]
    [XmlRoot("WindowsNetwork")]
    public class SessionWindowsNetworkSource : SessionSource
    {
        ...
        public SessionWindowsNetworkSource()
            : base()
        {
        ...
        public SessionWindowsNetworkSource(string name, string source)
            : base(name, source)
        {
            this.Guid = System.Guid.NewGuid().ToString();
        }
        ...
        {
            List<NetworkBrowser._SERVER_INFO_100> computers = browser.getNetworkComputers();

            foreach(NetworkBrowser._SERVER_INFO_100 computer in computers)
            {
                SessionWindowsNetworkLeaf session = new SessionWindowsNetworkLeaf(computer.sv100_name);
                session.Port = 22;
                IPAddress[] ipAddresses = Dns.GetHostAddresses(computer.sv100_name);
    ...
    [XmlType("SessionWindowsNetworkLeaf")]
    [XmlRoot("Session")]
    public class SessionWindowsNetworkLeaf : SessionLeaf, IComparable, ICloneable
    {
        [XmlIgnore]
        public override bool IsReadOnly { get {return true;} }

        public SessionWindowsNetworkLeaf()
            : base()
        {
        }

        public SessionWindowsNetworkLeaf(string name)
            : base(name)
        {
        }

        public SessionWindowsNetworkLeaf(string name, string hostName, int port, ConnectionProtocol protocol, string sessionConfig)
        ...
        {
            return this.PopulateClone(new SessionWindowsNetworkLeaf());
        }
    }
